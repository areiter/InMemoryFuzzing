 ...
 *
 * $Id: dighosts.c,v 1.5 2006/03/10 19:25:47 vlad Exp $
 */
...
int sockgetchar(SOCKET sock, int timeosec, int timeousec){
 ...
 FD_ZERO(&fds);
 FD_SET(sock, &fds);
 if (select (sock+1, &fds, NULL, NULL, &tv)!=1) return EOF;
...
int main(int argc, char *argv[]){
 ...
 WSAStartup(MAKEWORD( 1, 1 ), &wd);
	...
	*hostend = '/';
	...
	if(send(sock, buf, strlen((char *)buf), 0) != (int)strlen((char *)buf)) return 9;
